...
const { PAD, paint, verifyData, maxKeyLen } = utils
let { EOL } = utils
...
module.exports = (data, opts) => {
  ...
  for (let i = 0; i < valLength; i++) {
    ...
    padChar = tmp.style ? tmp.style
      : style || paint(tmp.color || color)
    key = tmp.key
    line = padChar.repeat(ratioLength) + EOL
